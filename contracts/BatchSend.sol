...
contract BatchSend is Adminable {
    ...
    function setWhiteList(address[] calldata recipients_, uint[] calldata amounts_) onlyAdmin public returns (bool) {
        ...
        for(uint i = 0; i < recipients_.length; i++){
            ...
            whiteList[recipients_[i]] = amounts_[i];
            _recipientsList.add(recipients_[i]);

            emit AddedRecipient(recipients_[i], amounts_[i]);
        }
        return true;
    }

    function cleanWhiteList() onlyAdmin public returns (bool) {
        ...
        require(_recipientsList.length() == 0);
    ...
    function containsRecipient(address recipient_) public view returns (bool) {
        return _recipientsList.contains(recipient_);
    }

    function recipientsCount() public view returns (uint) {
        return _recipientsList.length();
    }
